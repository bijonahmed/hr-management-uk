                    ...
                    <div class="ps-3">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb mb-0 p-0">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Late Policy List</li>
                            </ol>
                        </nav>
                    </div>
                    <div class="ms-auto">
                        <div class="btn-group">
                            <Nuxt-link to="/rota/add-late-policy"><button type="button" class="btn btn-primary"><i class="bx bx-plus"></i>New</button></Nuxt-link>
                        </div>
                    </div>
                </div>
                <!--end breadcrumb-->
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control shift_code" placeholder="Shift Code" v-model="searchQuery.shift_code" @input="handleSearch">
                                </div>
                            </div>
                            ...
                            <table class="table table-hover table-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Designation </th>
                                        <th>Shift Code</th>
                                        <th>Max Grade Period</th>
                                        <th>No. of Days Allow</th>
                                        <th>No. of Days Deducted</th>
                                ...
                                <tbody>
                                    <tr v-for="item in paginatedData" :key="item.id">
                                        ...
                                        <td>{{ item.max_grade_period }}</td>
                                        <td>{{ item.no_days_allow }}</td>
                                        <td>{{ item.day_salary_deducted }}</td>
                                        ...
                                        <td>
                                            <center>
                                                <nuxt-link :to="{name: 'rota-editLatePolicy-id', params: {id: item.id}}" variant="warning" size="sm"><i class="bx bx-edit"></i>EDIT</nuxt-link>
    ...
    export default {
        ...
        computed: {
            ...
            filteredData() {
                ...
                if (this.searchQuery.shift_code) {
                    result = result.filter(item =>
                        item.shift_code.toLowerCase().includes(this.searchQuery.shift_code.toLowerCase())
                    );
                }
        ...
        methods: {
            async fetchData() {
                ...
                try {
                    const response = await this.$axios.get(`/rota/getAllLatePolicy`);
